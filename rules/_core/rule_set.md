...
source: EHC_Checker_RULE_SET_v4_8.docx
version: 4.8
...
description: Core layer — universal rules that apply to every EHC regardless of route or commodity. Sections A2, A2.1, A6 are in the route layer (A10 Windsor Framework removed at v4.0); A12 and A13 are in JSON libraries. v4.0 changes (editorial review and rationalisation): gross weight non-tanker = hard error; bulk tanker identified by I.27 Type of packaging; Rotterdam unaccompanied I.13/I.15 hard errors (E61); AMR date threshold universal (A11); I.15 Esbjerg/Rotterdam vessel tick = hard error; Concise Report as default mode; A5.1 schedule pages universal rule; A10 Windsor Framework deleted (Severity Levels promoted from A11 to A10). v4.1 changes: A10 amended (typographical errors in free-text fields = AMBER); B1 I.18 ambient rule rewritten (dry powder silent pass any route; bulk tanker hard error if Chilled/Frozen); B1 I.17 amended with label-only vs valid examples lists; A7 small signing-box note added for 8322; A7.1 reformatted as 4×4 matrix; A8 reformatted as bullet list; A9 signing-date-not-today row added. v4.1.3: engine §2.6 added — calibration authority is binding (silent-pass calibrations emit zero flags; severity caps cannot be escalated by the engine on the same event). v4.5 changes: B1 I.1 UK-address rule (XI = AMBER, non-UK = RED); B1 I.12 EU/XI/CH-address rule (GB = RED); B1 I.13 Felixstowe added as confirmed exit port; I.22 Switzerland I.21-strike convention (E71); I3 §6 recommendations filter (surface only new importers, route/lane changes, genuine errors). v4.7 changes: B1 I.11 amended — GB DE 030 required at I.11 on AFI Taw Valley bulk tanker loads, hard error if absent (E73), and I.11 = Gregory Distribution North Tawton, approval N/A, on AFI powder loads (I.27 Type of packaging ≠ BULK TANKER); B1 I.12 approval-number sub-table row renamed to "8468 — AFI bulk tanker to Denmark (Danmark Protein, M119)" and confirmed 8468-only, with WPC and LPC accepted as interchangeable product descriptions rather than a certificate-type distinction. v4.8 changes: A11 rewritten — 8436 removed from II.1a scope entirely (Part K carries no such clause; wrongly listed since v2.5) and the AMR threshold is now live, keyed to the certificate SIGNING DATE; II.1a becomes RETAIN on 8468 / 8384 / 8350EHC COMP / 8471. D5: the II.2.2 raw milk zone-code row removed — referential to II.2.1, not a template blank. E74 (photo/document vehicle ID mismatch: RED on detection under B4, no inference-based self-resolution), E75 (compulsory field verification box by box) and E76 (scan degradation on address fields distinguished from A10) added. H1: North Tawton U1183488/TRANS unkeyed from consignor; Heathfield split into two roles, with U1435456/TRANS for AFI Taw Valley Cat 3 powder dispatch. H2/H3: Van Lee Melkprodukten BV and Arion Dairy Products BV added, Maersk name-equivalence confirmed, Puratos I.12 role stated. Version history table backfilled for 4.2–4.7, which had never been recorded here.
---
...
| **Field** | **Rule** |
|----|----|
| I.11 Place of dispatch | Name and address required. Approval number required — see H1 establishment library. Gregory Distribution, North Tawton, EX20 2EB: U1183488/TRANS — Cat 3 transport approval, applies to ANY consignor's 8322 Cat 3 load dispatched via this depot, not to one consignor's lane. Gregory Distribution, any other address: N/A — loading/logistics only. Heathfield: N/A — loading/logistics only on Saputo/Davidstow loads, EXCEPT AFI Taw Valley Cat 3 powder loads (bagged / bulk-bag, not bulk tanker) dispatched from Heathfield, which carry U1435456/TRANS (see E46). For 8436: PHS approval number required — medium warning if absent. If completely blank = low notice only. AFI Taw Valley bulk tanker loads (I.27 Type of packaging = BULK TANKER): GB DE 030 required at I.11 — hard error if absent (E73). AFI Taw Valley powder loads (I.27 ≠ BULK TANKER): I.11 = Gregory Distribution, North Tawton, EX20 2EB, approval N/A. **NOTE:** I.11 is the dispatch establishment. I.13 is logistics/loading address only — no approval number required or expected at I.13. |
...
| 8468 — AFI bulk tanker to Denmark (Danmark Protein, M119) | This lane is 8468 human consumption only — never 8322 Cat 3 on bulk tanker. Product description may appear as WPC or LPC on the EHC; both are accepted, not a scope restriction (cf. E18). Destination approval number required at I.12 — hard error if absent. EHC typically redacted so field rarely blank; flag only if visibly empty. See H3 M119, E64 and E73. |
...
| 4.2 | June 2026 | Library additions: CLASADO LIMITED (H2 I.1 consignor, Saputo Davidstow GOS lane); MEGGLE GMBH & CO. KG (H2 I.5 consignee, H3 I.12 destination, 8468 N/A). New trade lane: Clasado/Saputo Davidstow GOS syrup to Germany via Calais. |
| 4.3 | June 2026 | E61 amended: Saputo/Davidstow Rotterdam lane - Heathfield in I.13 confirmed as standing practice, not a hard error. E66 added: Jonker & Schut Barneveld approval number extra-zero typographical error pattern. Library additions: NUMIDIA B.V. (H2 I.5 consignee); HAVERO HOOGWEGT (H2 I.5 consignee). SKILL.md updated to v7.1. |
| 4.4 | June 2026 | F4b amended: GB BSE Negligible Risk reclassification (APHA BN49/25, effective 24 Nov 2025) — RUM_BOV_OV inner ‘either’ now retained; sub-options (a)(b)(c) remain deleted. Previous note stating inner ‘either’ cannot be retained deleted in full. E67 added. APHA BN49/25 PDF added to project files. RED flag on EHC 26/2/167861 (Forthglade/Renske 1501968 session) withdrawn — load cleared to PASS. |
| 4.5 | July 2026 | B1 I.1 UK-address rule (XI = AMBER, non-UK = RED); B1 I.12 EU/XI/CH-address rule (GB = RED). C4 whey permeate deletion corrected — retain ‘the milk-based products’; previous ‘all three struck’ certified nothing. C3/H4 Bech Gruppen I.25 = Animal feedingstuff. E61 amended: Felixstowe added as confirmed exit port. E68 I.11 vs schedule cross-check; E69 foreign-page / duplication; E70 Cancel-and-Replace date logic; E71 Switzerland I.21/I.22. Library: H2 Fayrefield, Makeway, Interfood; H3 Fayrefield, Pelican Rouge; H4 North Down Grain (names Ballywalter row); H7 CH I.21 strike. I3 §6 recommendations filter. Silvia API build sync required — C4 priority. |
| 4.6 | July 2026 | H1: Gregory Distribution, Ernesettle Lane, Plymouth (rare cheese depot, ~every 6 months) and Cullompton (occasional cheese depot) added. H2: County Milk I.5 self-account transfer pattern confirmed (Leprino Foods NI lane); Novades BV dual-route confirmed (Purfleet/Zeebrugge + Felixstowe/Rotterdam); Ingredient Solutions Ltd added; Allied Fleet Services added; Saputo Davidstow cheese-lane consignee/depot note added. H3: Leprino Foods Ltd Storage Account, Lactoprot Deutschland GmbH (DE SH 005), DSV Transport P.I. added. E12 amended (seal number controlling on “N/A UNWITNESSED” pattern). E72 added (Belfast/County Milk I.15 vessel tick, AMBER). |
| 4.7 | August 2026 | H1: Gregory Distribution North Tawton added as standard I.11 for AFI powder loads (N/A), distinct from Taw Valley Creamery/GB DE 030 tanker loads. E62 amended: "curtainsider" wording removed. B1/E64/H3: AFI Esbjerg lane reworded LPC/WPC-agnostic. E73 added: AFI tanker lane I.11 GB DE 030 blank = HARD error. D5 corrected: "II.1 Country of origin" and "II.4/II.5" rows removed (no such fields on 8468 template). F3 corrected to align with F4b (RUM_BOV_OV inner 'either' retained). H2: Arla Foods Ingredients Viby-J address corrected to "Soenderhoej 10-12"; Gobia Ltd / Dairy Consumer Foods confirmed 8468-only. H3: DSV Transport P.I. entry strengthened — any name/address deviation at this destination is a HARD error. |
| 4.8 | Sept 2026 | A11/D4/E3/J7/L3: AMR threshold live (3 Sept 2026) — deleted/redacted = hard error; 8436 removed from scope. D5: II.2.2 row removed — referential to II.2.1. E74 added: photo/document vehicle ID mismatch RED on detection, no inference-based self-resolution. E75 added: compulsory field verification, explicit box-by-box check. E76 added: scan degradation on address fields distinguished from A10. H1: North Tawton U1183488/TRANS corrected to any consignor; Heathfield corrected — AFI approval U1435456/TRANS added. H2: Van Lee Melkprodukten BV, Arion Dairy Products BV added; Maersk Customs Services name-equivalence confirmed. H3: Puratos Nederland N.V. added. |
