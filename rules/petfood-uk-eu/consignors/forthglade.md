...
| **F4b RUM_BOV_OV** | II.6 outer "either" deleted; II.6 "or" header retained; inner "(2) either" **RETAINED**; sub-options (a) (b) (c) deleted | **HARD** | CORRECTED v4.8. This row previously read "all four sub-options deleted — GB has had indigenous BSE cases, inner 'either' cannot be retained". That is RETIRED: GB is BSE Negligible Risk per APHA Briefing Note 49/25, effective for certificates signed on or after 24 November 2025, so the inner "(2) either" must be retained — see F4b in types/8324.md and calibration note E67. A flag raised against a retained inner "either" is a FALSE POSITIVE. This dossier was written at v4.2 and never re-synced, so it carried the withdrawn wording as a HARD rule — and the consignor section is appended LAST in the composed prompt, making it the final word the model read on exactly this clause. The RED on cert 26/2/167861 (Forthglade/Renske) was withdrawn for this reason |
