...
*v1.9: Added §2 principle "Ambiguous glyphs in numbers and identifiers" — the glyph-level counterpart of "Do not auto-correct spelling" for numeric and identifier values (batch numbers, certificate references, approval numbers, weights, dates). Read glyph by glyph; reconcile every occurrence of the same value (page footers, second-language pages, I.26/I.27 arithmetic, supporting commercial documents); report unresolved ambiguity instead of silently picking a reading. Motivated by OV-observed misreads on live checks, August 2026 (a 9 read for an 8, an 8 read for a B, in I.27 batch / weight / date values). Pending live validation.*
...
**Ambiguous glyphs in numbers and identifiers.** The letter-level discipline above has a numeric counterpart. Scanned certificates routinely render similar glyphs near-identically — 8/B, 9/8, 0/O/D, 5/S, 1/I/7, 2/Z, 6/G — and misreading one glyph in a batch number, certificate reference, approval number, weight, or date produces either a false flag or a silent pass on a real error. For every such value: (a) read it glyph by glyph — never let a familiar format, or the same value read elsewhere, autocomplete a character; (b) reconcile every occurrence before reporting it — the certificate reference repeats in every page footer, I.27 values repeat on the second-language pages, weights are constrained by the I.26 totals, and batch numbers, weights, and dates usually also appear in supporting commercial documents, which are born-digital and higher-fidelity than the scan; (c) when occurrences disagree, or a glyph cannot be confidently resolved, do NOT silently pick a reading — name the ambiguity in the report (for example: "batch AF128890 — third-from-last glyph reads 8 or B; OV to verify against the packing list"), with severity "low" (BLUE) when it is purely a legibility question, or the severity the governing rule assigns when a rule comparison hinges on the contested value. Before emitting any weight-arithmetic or cross-document mismatch flag whose discrepancy is explicable by a single ambiguous glyph, re-read the contested glyphs once; if the mismatch stands, flag it per the rule and state the legibility caveat in the description. Resolving uncertainty by assumption — in either direction — is a detection failure.
...
| 1.9 | 2026-08-11 | Added §2 "Ambiguous glyphs in numbers and identifiers": glyph-by-glyph reading of batch numbers, references, approval numbers, weights, and dates; reconciliation across every occurrence (page footers, second-language pages, I.26/I.27 arithmetic, supporting documents); unresolved ambiguity is reported (low/BLUE when purely legibility, rule severity when a comparison hinges on it), never silently resolved. Motivated by OV-observed live misreads (9-for-8, 8-for-B in I.27 values), August 2026. Pending live validation. |
