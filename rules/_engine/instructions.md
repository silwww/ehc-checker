...
*v1.9: Added §2 principle "Ambiguous glyphs in numbers and identifiers" — the glyph-level counterpart of "Do not auto-correct spelling" for numeric and identifier values (batch numbers, certificate references, approval numbers, seal numbers, weights, dates). Read glyph by glyph; reconcile every occurrence of the same value (page footers, second-language pages, I.26/I.27 arithmetic, supporting commercial documents); report unresolved ambiguity instead of silently picking a reading. Explicit boundaries hold the line against the auto-correct failure mode: reconciliation resolves perception uncertainty only and never overrides a confidently read certificate value (the certificate is the legal document the BCP reads); a motivated re-read that changes a reading dissolves a mismatch only into a mandatory legibility notice, never silently; §2.6 calibration authority keeps precedence; "No alternative interpretations" carries a stated carve-out; `legibility` joins the §2.5 category set. Motivated by OV-observed misreads on live checks, August 2026 (a 9 read for an 8, an 8 read for a B, in I.27 batch / weight / date values). Pending live validation.*
...
**Ambiguous glyphs in numbers and identifiers.** The letter-level discipline above has a numeric counterpart. Scanned certificates routinely render similar glyphs near-identically — 8/B, 9/8, 0/O/D, 5/S, 1/I/7, 2/Z, 6/G — and misreading one glyph in a batch number, certificate reference, approval number, seal number, weight, or date produces either a false flag or a silent pass on a real error. For every such value: (a) read it glyph by glyph — never let a familiar format, or the same value read elsewhere, autocomplete a character; (b) reconcile every occurrence before reporting the value — the certificate reference repeats in every page footer, I.27 values repeat on the second-language pages, weights are constrained by the I.26 totals, and batch numbers, weights, and dates usually also appear in supporting commercial documents. Reconciliation resolves PERCEPTION uncertainty only: it decides which glyph the scan shows where your own reading is uncertain. It never overrides a confidently read certificate value — the certificate, not the packing list, is the legal document the BCP reads. If the certificate confidently shows a value that disagrees with a supporting document, that is a cross-document discrepancy to flag at its rule severity, exactly as before this principle existed. (c) When your own reading of a certificate glyph remains uncertain — occurrences disagree, or the glyph cannot be confidently resolved — do NOT silently pick a reading: name the ambiguity in the report (for example: "batch AF128890 — third-from-last glyph reads 8 or B; OV to verify against the packing list"). "Purely a legibility question" means exactly this case — the certificate glyph itself cannot be confidently read — and only then is the severity "low" (BLUE, category `legibility`); when a rule comparison hinges on the contested value, use the severity the governing rule assigns. Before emitting any weight-arithmetic or cross-document mismatch flag whose discrepancy is explicable by a single ambiguous glyph, re-read the contested glyphs once. If the mismatch stands, flag it per the rule and state the legibility caveat in the description. If the re-read CHANGES a reading, the glyph was by definition ambiguous: the mismatch may dissolve only into the mandatory legibility notice above, never silently — a re-read performed knowing which reading balances the arithmetic must not be trusted as newly confident. Calibration authority (§2.6) takes precedence over this principle: where a calibration note silently passes or caps the value in question, do not add a legibility notice on top of it — the mandatory notice above applies only when no calibration note governs the contested value. A calibration governs only when its applicability is itself beyond doubt: if the contested glyph is what decides whether the calibration applies (for example, a batch prefix that selects between the AQ root-truncation pass and the AF full-batch requirement), then no calibration governs the value and the notice stands. Resolving uncertainty by assumption — in either direction — is a detection failure.
...
**No alternative interpretations.** Do not narrate options you considered. Do not say "this could be X or Y; I have chosen Y". Pick the correct interpretation per the rule set and report it. The single exception is an unresolved glyph ambiguity under "Ambiguous glyphs in numbers and identifiers" above: there the ambiguity IS the finding, and it is reported as such — never resolved by picking a reading.
...
- `legibility` — unresolved glyph ambiguity in a printed value (§2 "Ambiguous glyphs in numbers and identifiers")
...
| 1.9 | 2026-08-11 | Added §2 "Ambiguous glyphs in numbers and identifiers": glyph-by-glyph reading of batch numbers, references, approval numbers, seal numbers, weights, and dates; reconciliation across every occurrence (page footers, second-language pages, I.26/I.27 arithmetic, supporting documents); unresolved ambiguity reported (low/BLUE `legibility` when the certificate glyph itself cannot be confidently read, rule severity when a comparison hinges on it), never silently resolved. Boundaries: reconciliation resolves perception uncertainty only, never overrides a confident certificate reading; a motivated re-read dissolves a mismatch only into a mandatory legibility notice; §2.6 keeps precedence; "No alternative interpretations" carve-out stated; `legibility` added to §2.5 categories. Motivated by OV-observed live misreads (9-for-8, 8-for-B in I.27 values), August 2026. Pending live validation. |
