...
The full report is the same verification rendered at audit length; it is rendered client-side
on demand from this same single-call payload (checklist + this Checks Performed section), not
regenerated by a separate model call. Concise must not under-report relative to it.
...
async function runCheckStreamAttempt({ params, meta, onEvent, signal }) {
  ...
  // verdict or final_report — no partial report reaches the client.
  // Routes through the same SSE error pathway as CERT_TYPE_REQUIRED.
  if (finalMessage.stop_reason === 'max_tokens') {
    const err = new Error('Report was cut off because generation hit the token limit. The report is incomplete and not valid for certification — please re-run the check.');
    err.code = 'REPORT_TRUNCATED';
    throw err;
  }
  ...
  if (meta.checklistRows) {
    ...
    // Per-severity cross-check: the empty-flags check above misses the case
    // where flags is non-empty but under-counts a specific severity (e.g. a
    // checklist row filled HARD alongside only a `low` flag) — exactly the
    // silent false-PASS shape this project fears, since counters.hard_errors
    // would read 0 with no warning at all. Compare per severity instead of
    // only against flags.length. LOG ONLY — flags/counters stay authoritative.
    const filledChecklist = (report.checklist && typeof report.checklist === 'object') ? report.checklist : {};
    const verdictOf = (rowId) => {
      const entry = filledChecklist[rowId];
      const v2 = entry && entry.verdict;
      return typeof v2 === 'string' ? v2.toUpperCase() : v2;
    };
    const hardRowIds = v.findingRowIds.filter((id) => verdictOf(id) === 'HARD');
